...
tmp.expr <- data.frame(me$HiSeqV2.gz)
rownames(tmp.expr) <- tmp.expr$sample
tmp.expr <- tmp.expr[,!colnames(tmp.expr)=="sample"]
colnames(tmp.expr) <- gsub(".","-",colnames(tmp.expr),fixed=TRUE)
tmp.me <- tmp.expr
...
tmp.met <- data.frame(mm)
rownames(tmp.met) <- tmp.met$sample
tmp.met <- tmp.met[,!colnames(tmp.met)=="sample"]
colnames(tmp.met) <- gsub(".","-",colnames(tmp.met),fixed=TRUE)
tmp.mm <- tmp.met
...
tmp.clin <- data.frame(cli)
tmp.clin <- tmp.clin[tmp.clin$sample_type=="Primary Tumor",]
sampleinfo <- tmp.clin
...
mm <- mm[sample(c(1:nrow(mm)),size=20000,replace=FALSE),]
me <- me[sample(c(1:nrow(me)),size=5000,replace=FALSE),]
